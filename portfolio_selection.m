...
%Teil 2.1.2
%%
%Berechnen der diskreten Returns mit einer abge�nderten Funktion von
...
%%
%Teil 2.1.4
% Erstellen eines Histogramm f�r die Korrelationswerte
...
%%
%Teil 2.1.5
%Finden der Unternehmen mit der h�chsten Korrelation

[row,col]=find(CorDax{:,:} == max(CorDax2));
TickerSymb=CorDax.Properties.VariableNames(row);

%Anzeigen der Unternehmen mit der h�chsten Korrelation
% display table
fprintf('\nDie Tickersymbole mit der h�chsten Korrelation sind:\n')
celldisp(TickerSymb)
fprintf('\nIhre Korrelation betr�gt:\n')
...
%Simulieren von Gewichten f�r das Portfolio
weights=rand(200,1);
...
%Aufbereitung der Kovarianzen um Matrixmultiplikation machen zu k�nnen
xtemp = triu(cov1,1);
ztemp=xtemp(xtemp~=0);
